    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ background: C.card, borderBottom: `1px solid ${C.border}`, padding: "0 32px", display: "flex", alignItems: "center", height: 60, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 2, flex: 1 }}>
          {[{k:"queue", l:`Queue (${myLeads.length})`}, {k:"stats", l:"Stats"}, {k:"callbacks", l:`Callbacks (${myPending.length})`}, {k:"contacts", l:"Directory"}].map(t => (
            <button key={t.k} onClick={() => setTab(t.k)} style={{ background: "none", border: "none", color: tab === t.k ? C.text : C.muted, borderBottom: tab === t.k ? `2px solid ${C.brand}` : "2px solid transparent", padding: "0 16px", cursor: "pointer", fontWeight: 600 }}>{t.l}</button>
          ))}
        </div>
      ...
      <div style={{ padding: "32px 36px", maxWidth: 1280, margin: "0 auto" }}>
        {tab === "queue" && (
          <div>
            ...
            {myLeads.length === 0 ? <div style={{ textAlign: "center", padding: 60, color: C.muted }}>Queue empty!</div> : myLeads.map(lead => (
              <div key={lead.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderLeft: `4px solid ${C.brand}`, borderRadius: 12, padding: "18px 24px", marginBottom: 14, display: "flex", alignItems: "center", gap: 20 }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontWeight: 800, fontSize: 17 }}>{lead.name}</div><div style={{ color: C.muted, fontSize: 13 }}>{lead.phone} • {lead.customer_type} • {lead.assigned_promo}</div></div>
...
function AdminPage({ contacts, calls, agents, promos, onRefresh, onLogout, showToast }) {
  ...
  async function deletePromoCascade(id, name) {
    ...
    try {
      await db.deleteWhere("call_logs", "promo_name", name); // wipe calls
      await db.deleteWhere("contacts", "assigned_promo", name); // wipe leads
      await db.delete("promotions", id); // wipe promo
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ background: C.card, borderBottom: `1px solid ${C.border}`, padding: "0 32px", display: "flex", alignItems: "center", height: 60, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}><Btn onClick={() => setShowLogCall(true)} color={C.text} textColor={C.bg}>+ Log Call</Btn><button onClick={onLogout} style={{ background: "none", border: "none", color: C.muted, cursor: "pointer" }}>Logout</button></div>
      </div>
