...
function AgentPage({ user, contacts, calls, agents, promos, onRefresh, onLogout, showToast }) {
  ...
  // Base leads assigned to this agent
  ...
  const TABS = [
    { key: "queue", label: `My Queue (${baseLeads.length})` },
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.text, fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div style={{ background: C.card, borderBottom: `1px solid ${C.border}`, padding: "0 32px", display: "flex", alignItems: "center", height: 60, position: "sticky", top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <Btn onClick={() => setShowLogCall(true)} style={{ background: C.text, color: C.bg, border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Log Call</Btn>
        ...
        {tab === "queue" && (
          <div>
            ...
            {displayedLeads.length === 0
              ? <div style={{ background: C.card, borderRadius: 12, padding: 80, textAlign: "center", color: C.muted, border: `1px dashed ${C.border}`, fontSize: 15 }}>{baseLeads.length > 0 ? "No leads match this filter." : "🎉 Queue is empty! No pending leads assigned to you."}</div>
